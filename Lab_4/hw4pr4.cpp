...
try
{
	...
	d1.m=Date::Month(tm);
	cin>>d1.d;
 Date	d2=d1; //d2 == old value of d1
  
  Date d3 = d1++; //d3 == old value of d1, d1 == new value of d1  
  d1++;
  cout<<d1<<d2<<d3;

   if(d2 != d3)error("error 1 in postfix ++");
   if(d2 == d1)error("error 2 in postfix ++");
   if(d1 != ++d2)error("error 3 in postfix ++");
   cout << "Tomorrow is " << d1 << '\n';
