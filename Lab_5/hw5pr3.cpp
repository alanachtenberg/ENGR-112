...
{		a.d=(a.d+1);
		...
		if (!is_date(a.year(),a.month(),a.day())){
			a.d=1;
			if (a.month()==12){
			a.add_year(1);
			a.m=a.jan;}
...
try
{
	cout<<"Input Dates in the format year month day Example:(1980 1 1)\n";
	...
	D1.add_month(2);
	D1.add_day(2);
	string sd=to_string(D1.d);
	string sy=to_string(D1.y);
	int month_case=D1.m;
	string sm;
	switch(month_case){
	...
	vector<string> long_form{sm,sd,,sy};
	cout<<"2 months and 2 days from now is "<<long_form[0]<<" "<<long_form[1]<<", "<<long_form[2];
	}
